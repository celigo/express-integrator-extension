...
module.exports = function (app, options) {
  ...
  function validateRequest (req, res, next) {
    ...
    } else if (typeof req.body.function !== 'string') {
      errorList.push(errors.get('ROUTES_FUNCTION_TYPE_STRING'))
    }
  ...
  function callFunction (req, res, next) {
    ...
    if (!object) {
      return res.status(422).json({
        errors: [{ message: 'The object ' + req.body.type + " doesn't exist in the module.", code: 'object_not_found' }]})
    }
    ...
    func(requestOptions, function (err, result) {
      ...
      var error = util.validateAndReturnResponse(req.body.maxResponseSize, result)
