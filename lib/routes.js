...
module.exports = function (app, options) {
  ...
  function validateRequest (req, res, next) {
    ...
    if (systemToken !== options.systemToken) {
      ...
      return res.status(401).json({ errors : [errorSource.get('ROUTES_INVALID_SYSTEM_TOKEN')] })
    }

    if (req.body.diy) {
      if (!options.diy) {
        errors.push(errorSource.get('ROUTES_DIY_DOESNT_EXIST'))
      }
    } else if (req.body._connectorId) {
      if (!options.connectors || !options.connectors[req.body._connectorId]) {
        errors.push(errorSource.get('ROUTES_CONNECTOR_ID_DOESNT_EXIST'))
      }
    } else {
      errors.push(errorSource.get('ROUTES_REQUIRED_DIY_OR_CONNECTOR_ID_FIELD'))
    }

    if (!req.body.type) {
      errors.push(errorSource.get('ROUTES_MISSING_TYPE_FIELD'))
    }

    if (!req.body.options && !req.body.postBody) {
      errors.push(errorSource.get('ROUTES_MISSING_OPTIONS_FIELD'))
    }

    if (!req.body.function) {
      errors.push(errorSource.get('ROUTES_MISSING_FUNCTION_FIELD'))
      ...
      errors.push(errorSource.get('ROUTES_FUNCTION_EMPTY_ARRAY'))
    }
  ...
  function callFunction (req, res, next) {
    ...
    if (!_.isFunction(func)) {
      return res.status(422).json(
        { errors : [ errorSource.get('ROUTES_INCORRECT_FUNCTION') ] })
    }
